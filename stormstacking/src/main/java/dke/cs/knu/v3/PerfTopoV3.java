...
public class PerfTopoV3 {

    private static final String TOPOLOGY_NAME = "PerformanceTopo";
    private static final String FIRST_SPOUT_ID = "random-spout";
    private static final String LEVEL0A_BOLT_ID = "level0a-bolt";
    private static final String LEVEL0B_BOLT_ID = "level0b-bolt";
    private static final String LEVEL0C_BOLT_ID = "level0c-bolt";
    private static final String LEVEL1_BOLT_ID = "level1-bolt";
    private static final String LAST_BOLT_ID = "last-bolt";

    private static Log LOG = LogFactory.getLog(PerfTopoV3.class);
    ...
    private static StormTopology getTopology(Map conf) {
        ...
        BoltDeclarer middleBolt = builder.setBolt(LEVEL0A_BOLT_ID, new Level0_A_ModelBolt(modelPath), Helper.getInt(conf, "middle.bolt.executors.count", 1))
                .setNumTasks(Helper.getInt(conf,"middle.bolt.tasks.count", 1));
        setGrouping(middleBolt, conf, prevTaskID);
        prevTaskID = LEVEL0A_BOLT_ID;

        BoltDeclarer middle2Bolt = builder.setBolt(LEVEL0B_BOLT_ID, new Level0_B_ModelBolt(modelPath), Helper.getInt(conf, "middle.bolt.executors.count", 1))
                .setNumTasks(Helper.getInt(conf,"middle.bolt.tasks.count", 1));
        setGrouping(middle2Bolt, conf, prevTaskID);
        prevTaskID = LEVEL0B_BOLT_ID;

        BoltDeclarer middle3Bolt = builder.setBolt(LEVEL0C_BOLT_ID, new Level0_C_ModelBolt(modelPath), Helper.getInt(conf, "middle.bolt.executors.count", 1))
                .setNumTasks(Helper.getInt(conf,"middle.bolt.tasks.count", 1));
        setGrouping(middle3Bolt, conf, prevTaskID);
        prevTaskID = LEVEL0C_BOLT_ID;

        BoltDeclarer middle4Bolt = builder.setBolt(LEVEL1_BOLT_ID, new Level1Bolt(modelPath), Helper.getInt(conf, "middle.bolt.executors.count", 1))
                .setNumTasks(Helper.getInt(conf,"middle.bolt.tasks.count", 1));
        setGrouping(middle4Bolt, conf, prevTaskID);
